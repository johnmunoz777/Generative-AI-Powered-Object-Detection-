...
def get_license_plate_text_multi_ocr(img, x1, y1, x2, y2):
    """
    Try multiple OCR engines and return the best result automatically
    """
    ...
    contrast = cv2.convertScaleAbs(gray, alpha=2.0, beta=10)
    ...
    for desc, enhanced in enhanced_images:
        ...
        # Method 2: Tesseract with different configs
        tesseract_configs = [
        ...
        for config in tesseract_configs:
            try:
                ...
                if len(clean_text) >= 3:
                    ...
                    confidence = min(0.9, len(clean_text) / 8.0 + 0.3)
                    ocr_results.append({
                        'text': clean_text,
                        'confidence': confidence,
    ...
    if ocr_results:
        ...
        # Print the results for debugging/logging purposes (optional)
...
img = cv2.imread("test_videos/lisn_test.jpg")
...
for r in results:
    ...
    for box in boxes:
        ...
        if currentClass == 'license_plate':
            ...
            # Use multi-OCR function - now returns best result automatically
            license_plate_text, status = get_license_plate_text_multi_ocr(img, x1, y1, x2, y2)
            
            if license_plate_text:
                license_plate_number = license_plate_text
                print(f"License plate detected: {license_plate_number}")
            else:
...
# Add title
if has_no_vest and license_plate_number:
    title = f"VIOLATION DETECTED - LICENSE: {license_plate_number}"
    title_color = (0, 0, 255)
elif has_no_vest and not license_plate_number:
    title = "VIOLATION DETECTED - LICENSE PLATE UNREADABLE"
    title_color = (0, 0, 255)
else:
    title = None
